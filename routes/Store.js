...
router.get('/store/:store_id', (req, res) => {
  ...
  db.query(
    `select store.*, store_detail.*, db_image.image from store 
  LEFT JOIN store_detail ON store_detail.store_id = store.store_id 
  LEFT JOIN db_image ON db_image.store_id = store.store_id
                ...
                db.query(
                  ...
                  (err, result1) => {
                    if (err) {
                      ...
                      db.query(
                        `insert into store_detail (store_id) values (${store_id})`,
                        (err, result2) => {
                          if (err) {
                            return res.status(401).send(err)
                          } else {
