                ...
                if (err) {
                  ...
                  data1.forEach(user => {
                    ...
                    if (
                      ...
                      user.item = Number(
                        user.item_number.toFixed(1)
                      ).toLocaleString()
                      itemss = itemss + user.item_number
                      user.price = Number(
                        user.order_price.toFixed(1)
                      ).toLocaleString()
                      prices = prices + user.order_price
                      user.cancel_detail = ''
                  ...
                  let row = worksheet.addRow({
                    no: 'รวม',
                    item: Number(itemss.toFixed(1)).toLocaleString(),
                    price: Number(prices.toFixed(1)).toLocaleString()
                  })
                  row.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'ff3cb35a' },
                    bgColor: { argb: 'ff3cb35a' }
                  }
                  ...
                  try {
                    const data = workbook.xlsx
                      .writeFile(`${path}/store_${user_id}.xlsx`)
                      .then(() => {
                        res.send({
                          status: 'success',
                          message: 'file successfully downloaded',
                          path: `${path}/store_${user_id}.xlsx`
                        })
                      })
                  } catch (err) {
                    res.send({
        ...
        if (err) {
          ...
          data1.forEach(user => {
            ...
            user.created_at = moment(user.created_at)
              .add(543, 'year')
              .format('D MMMM YYYY HH:mm:ss')
            if (user.order_user_cancel === 1 && user.order_store_cancel === 0) {
              user.status = 'ลูกค้ายกเลิก'
              user.cancel_detail = user.order_cancel_detail
            } else if (
              user.order_user_cancel === 0 &&
              ...
              user.status = 'กำลังดำเนินการ'
              user.item = Number(user.item_number.toFixed(1)).toLocaleString()
              itemss = itemss + user.item_number
              user.price = Number(user.order_price.toFixed(1)).toLocaleString()
              prices = prices + user.order_price
              user.cancel_detail = ''
              if (user.order_status === 0) {
                user.ok_status = 'รอยืนยัน'
